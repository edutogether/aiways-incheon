...
function createAnalysisIdempotency({ db, now = () => new Date(), serverTimestamp = () => new Date(), model }) {
  ...
  async function completeAnalysisRequest(actorId,key,result) { try { await db.runTransaction(async tx=>tx.update(refFor(actorId,key),{state:"completed",updatedAt:serverTimestamp(),result})); return true;}catch{return false;} }
  async function failAnalysisRequest(actorId,key,errorCode,httpStatus,retryAfterUntil) { try { await db.runTransaction(async tx=>tx.update(refFor(actorId,key),{state:"failed",updatedAt:serverTimestamp(),errorCode,httpStatus,...(retryAfterUntil?{retryAfterUntil}: {})})); return true;}catch{return false;} }
