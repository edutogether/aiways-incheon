...
const body = {
  ...
  image: {
    mimeType: "image/png",
    data: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADElEQVR42mNk+M/wHwAF/gL+QKQe2QAAAABJRU5ErkJggg=="
  },
  imageMetadata: {mimeType: "image/png", width: 1, height: 1, byteLength: 70},
  userContext: {}
};

async function call(appCheck) {
  let provider = 0;
  let limit = 0;
  let claim = 0;
  const h = createAnalyzeSortingHandler({
    ...
    analysisRequests: {
      ...
      completeAnalysisRequest: async () => true,
      failAnalysisRequest: async () => true
    },
    createClient: () => ({
      models: {
  ...
  const r = {
    set() { return this; },
    status(v) { this.s = v; return this; },
    json(v) { this.b = v; return this; },
    send() { return this; }
  };
  await h({method: "POST", headers: {}, body}, r);
...
(async () => {
  ...
  for (const [code, status] of [["app_check_missing", 401], ["app_check_invalid", 401], ["protection_unavailable", 503]]) {
    const out = await call(async () => ({status: "missing", httpStatus: status, code}));
    assert.equal(out.r.s, status);
    assert.equal(out.provider, 0);
