(() => {
  ...
  // 갖고 있으면 검색이 **0.4~1.5ms**로 끝나 "타자 치는 대로" 좁혀진다.
  //
  // 🔴 서버 검색(`searchSchool`)을 지우지 않는다 — 목록이 아직 안 왔거나
  // 못 받았을 때 **그쪽으로 떨어진다**(폴백). 그래서 느린 회선에서도
  // 지금보다 나빠지는 경우가 없다.
  //
  // 🔴 이 파일(app.js)은 **PC 대시보드 화면에서만** 실행된다. 학생 앱은
  // `mobile/`(별도 빌드)이라 이 목록을 받지 않는다 — 학생이 학교 검색을
  // 쓰지도 않는데 학교 와이파이에서 164KB를 받게 하면 안 된다.
  //
  // 🔴 파일명 뒤 `?v=`는 **내용 해시**다. `scripts/fetchSchoolList.js`가
  // 목록을 다시 만들 때 이 줄을 같이 고친다. 경로는 그대로라 `firebase.json`의
  // 1년 불변 캐시 규칙이 계속 맞고, 내용이 바뀌면 주소가 달라져 새로 받는다.
  ...
  // 회선이 느리다는 뜻은 아니다).
  function mayPrefetchSchoolList() {
  ...
  // 🔴 첫 화면을 늦추지 않는다 - 유휴 시간에, 낮은 우선순위로만 받는다.
  function prefetchSchoolList() {
    if (!mayPrefetchSchoolList()) return;
    const start = () => loadSchoolList();
    if (typeof window.requestIdleCallback === "function") window.requestIdleCallback(start, { timeout: 8000 });
  ...
  function searchSchoolListLocally(rawQuery) {
  ...
  function initDashboardSchoolSetup() {
    ...
    async function runSearch(rawQuery) {
      ...
      // 🔴 목록이 준비돼 있으면 서버를 거치지 않는다(0.4~1.5ms). 준비 전이거나
      // 받지 못했으면 아래 서버 검색으로 그대로 떨어진다 - 화면은 양쪽이 같다.
      const localHits = searchSchoolListLocally(query);
      if (localHits) {
        renderSchoolResults(localHits);
        return;
      }
      ...
      renderSchoolResults(response.data?.schools || []);
    }
    // 🔴 목록 검색과 서버 검색이 **같은 코드로** 그린다. 두 벌로 두면 한쪽만
    // 고쳐지는 날이 반드시 오고, 그때 "폴백일 때만 화면이 다른" 상태가 된다.
  ...
  function boot() {
    ...
    // 🔴 학교 목록을 **유휴 시간에 낮은 우선순위로** 미리 받아 둔다. 학교를
    // 이미 설정한 기기에서도 받는 이유는 톱니 메뉴의 "학년반 다시 설정하기"가
    // 같은 검색 화면을 열기 때문이다. 아낌 모드·2G면 건너뛴다(폴백으로 간다).
    prefetchSchoolList();
