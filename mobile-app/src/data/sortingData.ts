// 분리배출 안내 데이터. mobile/sortingData.js를 옮긴 것이다.
//
// 옮길 때 본문을 손으로 다시 치지 않았다 - 품목 안내 문구 370줄을 사람이
// 옮기면 오타 하나가 그대로 학생에게 잘못된 안내로 나간다. 원본 파일에서
// IIFE 껍데기만 벗겨 기계적으로 꺼낸 뒤 타입만 붙였다. 그래도 전사 오류가
// 없다는 보장은 안 되므로, sortingData.parity.test.ts가 **원본 파일을 실제로
// 실행해서** 이 모듈과 깊은 비교를 한다.
//
// 원본은 S5(라이브 교체)에서 지운다. 그때까지 두 벌이 공존하는 것은 라이브를
// 건드리지 않기 위해서다 - 지금 mobile/을 고치면 학생들이 쓰는 화면이 바뀐다.

// 판정 항목 하나. judgementItem()이 빈 배열·0으로 기본값을 채우므로, 실제로
// 쓰이는 값은 전부 채워져 있다.
...
// judgementItem()에 넘기는 값. 기본값이 있는 항목은 생략할 수 있고, id는
// 나중에 키에서 채워 넣는다.
type SortingItemConfig =
  Omit<SortingItem, "id" | "materialCandidates" | "disposalCandidates" | "visibleCautions" | "holdReasons" | "searchKeywords" | "carbonSaved">
  & Partial<Pick<SortingItem, "materialCandidates" | "disposalCandidates" | "visibleCautions" | "holdReasons" | "searchKeywords" | "carbonSaved">>;
...
// 검색창 아이콘 전용 이모지 표. [이모지, 그 이모지를 부르는 말들].
export type DecorativeEmojiEntry = readonly [string, readonly string[]];

// 퀴즈 문장 템플릿이 받는 값. 품목 이름과 분류, 그리고 "~한" 꼴로 문장에
// 끼워 넣는 동작 구절이다.
interface QuizTemplateItem {
  emoji: string;
  name: string;
  category: string;
  action: string;
...
function judgementItem(config: SortingItemConfig): SortingItem {
  return {
    id: "",
    materialCandidates: [],
    disposalCandidates: [],
...
const sortingDbV2: Record<string, SortingItem> = {
...
const QUICK_SELECT_ORDER: readonly string[] = ["milk-carton", "tape-box", "plastic-cup", "ramen-container", "snack-wrapper", "can", "pet-bottle", "glass-bottle", "paper-cup", "vinyl-bag", "receipt", "battery"];
...
const DECORATIVE_EMOJI_LOOKUP: readonly DecorativeEmojiEntry[] = [
  ["📖", ["책", "교과서", "동화책", "잡지"]],
  ["📓", ["공책", "노트", "수첩"]],
  ["✏️", ["연필", "샤프"]],
  ["🖊️", ["볼펜", "펜", "만년필"]],
...
function shuffle<T>(items: readonly T[]): T[] {
  ...
  for (let i = arr.length - 1; i > 0; i -= 1) {
    ...
    // i와 j는 모두 0..arr.length-1 안이라 undefined가 나올 수 없다.
    // 타입 검사기는 그걸 모르므로 여기서만 단언한다.
    const swapped = arr[i]!;
    arr[i] = arr[j]!;
    arr[j] = swapped;
  }
...
const object = (name: string) => (hasBatchim(name) ? "을" : "를");
...
function buildExpandedSortingQuizData(seedQuestions: QuizQuestion[]): QuizQuestion[] {
  const quizItems: readonly (readonly [string, string, string, string])[] = [
  ...
  const trueTemplates: ((item: QuizTemplateItem) => string)[] = [
  ...
  const falseTemplates: ((item: QuizTemplateItem) => string)[] = [
...
// 씨앗 문항. [이모지, 문장, 정답] 꼴로 적고 아래에서 설명 문구를 붙인다.
const seedQuizRows: readonly (readonly [string, string, boolean])[] = [
...
const quizPool = buildExpandedSortingQuizData(seedQuizRows.map(([emoji, question, answer]) => ({
  emoji, question, answer,
  explanation: answer ? "맞는 기준입니다. 실제 배출 전 오염 상태와 학교 기준을 한 번 더 확인해요." : "헷갈리기 쉬운 기준입니다. 재질과 오염 상태를 다시 살펴봐요."
})));

function pickQuizSet(): QuizQuestion[] {
...
export { sortingDbV2, QUICK_SELECT_ORDER, DECORATIVE_EMOJI_LOOKUP, quizPool, pickQuizSet, quizRank };
