(() => {
  ...
  // ── 전국 학교 목록 검색은 **공용 모듈**이 한다 (2026-09-11, 지시 Bumm)
  //
  // 🔴 구현은 루트의 `schoolListSearch.js` 한 곳에만 있다. PC 대시보드와 학생
  // 앱이 같은 검색을 쓰는데 두 벌로 두면 한쪽만 고쳐지는 날이 반드시 오고,
  // 그때 "학생 앱에서만 검색이 다른" 상태가 된다. `index.html`이 이 파일보다
  // **먼저** 그 스크립트를 읽는다.
  //
  // 🔴 서버 검색(`searchSchool`)을 지우지 않는다 — 목록이 아직 안 왔거나 못
  // 받았을 때 그쪽으로 떨어진다(폴백). 두 경로가 `renderSchoolResults()`
  // 하나로 그린다.
  const schoolList = () => window.AIWaysSchoolList;
  // 목록이 준비돼 있으면 그 자리에서 거른다. 없으면 null - 부르는 쪽이 서버로 간다.
  const searchSchoolListLocally = (query) => schoolList()?.search(query) ?? null;
  // 첫 화면을 늦추지 않는다 - 유휴 시간에, 낮은 우선순위로만 받는다.
  const prefetchSchoolList = () => schoolList()?.loadWhenIdle();
  ...
  function initDashboardSchoolSetup() {
    ...
    input.addEventListener("input", () => {
      ...
      if (schoolList()?.isReady()) { runSearch(input.value); return; }
