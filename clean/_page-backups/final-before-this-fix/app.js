(() => {
  ...
  function applyDashboard(records) {
    ...
    setText("[data-school-classes]", dashboard.schoolClasses.toLocaleString("ko-KR"));
    ...
    if (classBadge) classBadge.textContent = "선택 학급";
  ...
  function initGallery() {
    ...
    function openDetail(card) {
      ...
      detail.querySelectorAll("div span").forEach((slot, index) => {
        slot.textContent = `${title} 사진 슬롯 ${index + 1}`;
