"use strict";
// Runs only in demo-aiways-incheon emulators. It uses synthetic PASS values and
...
(async () => {
  ...
  const registry = createEdu2gPassRegistry({ getSecret: () => JSON.stringify({ version: 1, passes: [
    { pass: "TEST-PASS-ALPHA", actorId: "actor_alpha", displayName: "알파", enabled: true, maxDevices: 5 },
    { pass: "TEST-PASS-BETA", actorId: "actor_beta", displayName: "베타", enabled: true, maxDevices: 5 }
  ] }) });
  ...
  try {
    ...
    for (let index = 0; index < 4; index += 1) { const out = await call(handlers.redeem, tokens[index], { pass: "TEST-PASS-ALPHA", deviceLabel: `기기 ${index + 1}`, platform: "web" }); assert.equal(out.statusCode, 200); assert.equal(out.body.activeDeviceCount, index + 1); }
    const concurrent = await Promise.all([call(handlers.redeem, tokens[4], { pass: "TEST-PASS-ALPHA", deviceLabel: "기기 5", platform: "web" }), call(handlers.redeem, tokens[5], { pass: "TEST-PASS-ALPHA", deviceLabel: "기기 6", platform: "web" })]);
    ...
    const repeat = await call(handlers.redeem, tokens[0], { pass: "TEST-PASS-ALPHA", deviceLabel: "기기 1", platform: "web" }); assert.equal(repeat.body.alreadyRegistered, true);
    const rebind = await call(handlers.redeem, tokens[0], { pass: "TEST-PASS-BETA", deviceLabel: "기기 1", platform: "web" }); assert.equal(rebind.body.code, "device_already_bound");
    const beta = await call(handlers.redeem, tokens[deniedIndex], { pass: "TEST-PASS-BETA", deviceLabel: "베타 기기", platform: "web" }); assert.equal(beta.statusCode, 200);
    ...
    const all = await Promise.all([db.collection("actors").doc("actor_alpha").collection("trustedDevices").get(), db.collection("edu2gDeviceBindings").get()]); assert.equal(JSON.stringify(all.map(snap => snap.docs.map(doc => doc.data()))).includes("TEST-PASS"), false); assert.equal(JSON.stringify(all.map(snap => snap.docs.map(doc => doc.data()))).includes("idToken"), false);
