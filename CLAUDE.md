...
**`_docs/ops/handoff.md`와 이 파일의 역할 차이**: `_docs/ops/handoff.md`는 세션 간 인수인계용 상세 작업일지(진행 중인 이슈, 다음에 할 일, 미해결 버그의 시행착오 기록)라 오래되면 낡은 항목이 남아있을 수 있다(마지막 갱신 2026-08-19). `CLAUDE.md`(이 파일)는 저장소 전역 규칙·정착된 결정·구조 요약이며 새 세션이 항상 먼저 읽는 곳이다. 최신 상태 파악은 이 파일을 우선하고, 특정 기능의 구현 배경/시행착오가 궁금하면 `_docs/ops/handoff.md`를 참고할 것.
...
## 정기 종합감사 주기 (2026-09-11 Bumm님 확정 — COMMON_STANDARDS §25)

**정기 종합감사는 분기 말(3월 말·6월 말·9월 말·12월 말)에 돈다. 다음은 2026-09-30이다.**
여덟 저장소가 **같은 날짜**를 쓴다 — 저장소마다 날짜가 다르면 감사가 일제히 안 돈다.

- **수시 감사는 개발 중에 Bumm님이 필요하실 때 지시하신다.** 🔴 **세션이 먼저
  제안하지 않는다**(§19). 분기 말이 아닌 시점에 "이제 감사할 때가 됐다"고 세션이
  운을 떼지 않는다는 뜻이다.
- **종합감사가 10/10이 나오면 그것이 "앱 개발을 멈춰도 된다"는 신호다.** 그 뒤는
  운영이고, 사소한 수정이 쌓이면 그때 다시 돌리거나 다음 분기 말을 기다린다 —
  **둘 다 정당하다.**

(이 규칙이 왜 필요한가: 2026-09-11에 여덟 저장소를 훑어보니 **셋에만 날짜가 적혀
있고 다섯은 아무것도 없었다.** 아는 저장소와 모르는 저장소가 갈리는 상태 자체가
결함이라, 날짜 하나가 아니라 **주기를 규칙으로** 적어 둔다.)
...
1. **교사코드 인증**(`functions/lib/teacherAuth.js`) — **반 단위 코드**(문서 ID가 `학교코드_학년_반`)로 `verifyTeacherCode`가 `actors/{actorId}.teacherVerified`를 기록. 코드는 **scrypt + 코드마다 다른 랜덤 솔트**로 해싱해 저장하고 비교는 `timingSafeEqual`로 한다(초기엔 sha256이었으나 사람이 고른 짧은 문자열이라 빠른 해시가 사전공격에 취약해 교체). 2026-09-07부터 **반 단위 실패 잠금**도 걸려 있다 — 10분 안에 10회 실패하면 그 반의 코드 검증을 15분 차단한다(`registerStudentProfile`의 담임 분기도 같은 로직을 재사용하므로 어느 경로로 시도하든 공통 적용).
...
**2026-08-19~25, 백엔드 전면 재설계 완료.** 8/19 사용자가 라이브 상태를 직접 캐물으며 코드를 확인한 결과 "실제로 작동하는 시스템이 아니었음"을 발견(모바일 앱에 네트워크 호출이 0줄, Code.gs/Firestore 이중 백엔드, 화면에 뜨는 최종 판단을 Gemini가 아니라 MobileNet이 정하고 있던 버그 등 — 상세 경위는 `_docs/ops/handoff.md` -1절). 그 자리에서 확정한 새 방향을 8단계로 순서대로 구현 완료했다:
