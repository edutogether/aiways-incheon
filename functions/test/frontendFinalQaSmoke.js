...
const inspectExpression = `(async () => {
  ...
  const clipped = headings.filter(node => node.scrollWidth > node.clientWidth + 1 || node.scrollHeight > node.clientHeight + 1).length;
...
(async () => {
  ...
  const fixture = await evaluate(page, `(() => {
    ...
    const boxes=[...result.querySelectorAll('p,span,strong,button')]; return {ready:true, overflow:document.documentElement.scrollWidth===document.documentElement.clientWidth, clipped:boxes.filter(n=>n.scrollWidth>n.clientWidth+1||n.scrollHeight>n.clientHeight+1).length, modalClosed:!modal.open, close:Boolean(b&&b.width>=40&&b.height>=40), sources:(document.body.textContent.match(/AI 사진 분석 참고 후보/g)||[]).length};
  })()`);
